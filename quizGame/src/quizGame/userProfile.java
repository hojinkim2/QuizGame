...
public class userProfile {
    ...
    public static String[] quiz_Database = {
            "자바를 창시한 사람은 누구입니까?", "경북대 최고의 맛집은 어디입니까?", "중국어로 xingbake는 무슨 의미입니까?", "경북대 개교기념일은 언제입니까?",
            " '까르보나라'의 어원은?"
    };
    ...
    static String[][] quiz_Bogi = {
            { "리오넬 고슬링", "제임스 고슬링", "아이엠 고슬링", "로베르토 고슬링" }, { "브리또", "공대식당", "맘스터치", "유낭" },
            { "비속어 단어", "스타벅스", "바퀴벌레", "고양이" }, { "1월 1일", "5월 28일", "6월 1일", "12월 25일" },
            { "불가사리", "숯", "크림", "계란 노른자" }
    };
    ...
    static void goNext(int num) {
        ...
        if (life < 0)
            System.out.print("목숨을 다했다.\nYOU DIED");
        if (isCorrect[num] == true)
            userProfile.num++;
    }
    ...
    static void getGrade() {
        if (userProfile.life > 0)
            System.out.println("살아 남으셨습니다.\n수고 하셨습니다\n");
        if (userProfile.life >= 9)
            System.out.println("당신의 등급 :A");
        else if (userProfile.life >= 7)
            System.out.println("당신의 등급: B");
        else if (userProfile.life >= 5)
            System.out.println("당신의 등급: C");
        else
            System.out.println("당신의 등급: F");
    }
